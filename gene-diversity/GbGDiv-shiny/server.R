...
shinyServer(function(input, output) {
  ...
  df <- reactive({
    ...
    if ( "NonVarNuc" %in% colnames(df.all()) ) # if alignments were completed
    {
    ...
    { return ( df.all ) } # then df.all will all be copied into df
    else { # that, if there are some exclusions
      #  
  ...
  output$aligninc <- reactive({
    return("NonVarNuc" %in% colnames(df()))
  })
  outputOptions(output, 'aligninc', suspendWhenHidden=FALSE)
  ...
  df.sel <- reactive ({
    ...
    if ( "NonVarNuc" %in% colnames(df()) == TRUE ) { df.sel$Category <- df$Category }
  ...
  scatterPlot <- reactive({
    ...
    if ( input$colourVar == "None" ) {
      if (input$categ)
      {  
        df.sel$csel <- df.sel$Category 
        colourtitle <- "Categories"
      }
      else
      {
